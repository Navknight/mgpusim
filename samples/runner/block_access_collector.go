...
// cacheBlockAccessCountTracer is an interface extracted from the writeback package
// to avoid direct dependencies
type cacheBlockAccessCountTracer interface {
...
// blockAccessTraceProvider defines a component that can provide a block access tracer
type blockAccessTraceProvider interface {
	GetBlockAccessTracer() cacheBlockAccessCountTracer
}

// blockAccessTrace records cache block access patterns
type blockAccessTrace struct {
	cache             string
...
// blockAccessTraceCollector collects cache block access traces
type blockAccessTraceCollector struct {
	traces []blockAccessTrace
}

// newBlockAccessTraceCollector creates a new collector
func newBlockAccessTraceCollector() *blockAccessTraceCollector {
	return &blockAccessTraceCollector{}
}

// Collect gathers traces from each cache
func (c *blockAccessTraceCollector) Collect(cacheProvider blockAccessTraceProvider, cacheName string) {
	tracer := cacheProvider.GetBlockAccessTracer()
	if tracer == nil {
		panic("nil tracer")
	}
	...
	trace := blockAccessTrace{
		cache:             cacheName,
		readAccessCounts:  tracer.GetReadAccessCounts(),
		writeAccessCounts: tracer.GetWriteAccessCounts(),
		histogramBinSize:  tracer.GetHistogramBinSize(),
		totalEvictions:    tracer.GetTotalEvictions(),
	}
	if len(trace.readAccessCounts) == 0 && len(trace.writeAccessCounts) == 0 {
		panic("empty trace")
	}
	fmt.Println(trace)
...
// Dump writes the collected traces to files
func (c *blockAccessTraceCollector) Dump(dir string) {
	if len(c.traces) == 0 {
		panic("no traces")
	}
	...
	c.dumpCSV(dir)
	c.generatePythonScript(dir)
}

// dumpCSV saves the collected data as a CSV file
func (c *blockAccessTraceCollector) dumpCSV(dir string) {
	fmt.Println("printing csv")
	f, err := os.Create(filepath.Join(dir, "cache_block_access.csv"))
	if err != nil {
		panic(err)
	}
	defer f.Close()

	// Write header for cache info
	fmt.Fprintf(f, "Cache,BinSize,TotalEvictions\n")
	for _, trace := range c.traces {
		fmt.Fprintf(f, "%s,%d,%d\n",
			trace.cache,
			trace.histogramBinSize,
			trace.totalEvictions)
	}
	fmt.Fprintf(f, "\n")
	...
	maxBins := 0
	for _, trace := range c.traces {
		if len(trace.readAccessCounts) > maxBins {
			maxBins = len(trace.readAccessCounts)
		}
	}

	for i := 0; i < maxBins; i++ {
		binSize := c.traces[0].histogramBinSize
		lowerBound := i * binSize
		upperBound := lowerBound + binSize - 1
		if i == maxBins-1 {
			fmt.Fprintf(f, ",Read %d+", lowerBound)
		} else {
			fmt.Fprintf(f, ",Read %d-%d", lowerBound, upperBound)
		}
	}
	fmt.Fprintf(f, "\n")

	// Write read access data for each cache
	for _, trace := range c.traces {
		fmt.Fprintf(f, "%s", trace.cache)
		for i := 0; i < maxBins; i++ {
			if i < len(trace.readAccessCounts) {
				fmt.Fprintf(f, ",%d", trace.readAccessCounts[i])
			} else {
				fmt.Fprintf(f, ",0")
			}
		}
	...
	// Write write access data header
	fmt.Fprintf(f, "Cache")
	for i := 0; i < maxBins; i++ {
		binSize := c.traces[0].histogramBinSize
		lowerBound := i * binSize
		upperBound := lowerBound + binSize - 1
		if i == maxBins-1 {
			fmt.Fprintf(f, ",Write %d+", lowerBound)
		} else {
			fmt.Fprintf(f, ",Write %d-%d", lowerBound, upperBound)
		}
	}
	fmt.Fprintf(f, "\n")

	// Write write access data for each cache
	for _, trace := range c.traces {
		fmt.Fprintf(f, "%s", trace.cache)
		for i := 0; i < maxBins; i++ {
			if i < len(trace.writeAccessCounts) {
				fmt.Fprintf(f, ",%d", trace.writeAccessCounts[i])
			} else {
				fmt.Fprintf(f, ",0")
			}
		}
...
// generatePythonScript creates a Python script for visualization
func (c *blockAccessTraceCollector) generatePythonScript(dir string) {
	script := `# Cache Block Access Histogram Generation
...
`
	...
	scriptPath := filepath.Join(dir, "generate_block_access_histograms.py")
